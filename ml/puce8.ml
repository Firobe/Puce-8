...
let keyboard_map = [
    Sdl.Scancode.kp_0; Sdl.Scancode.kp_7;
    Sdl.Scancode.kp_8; Sdl.Scancode.kp_9;
    Sdl.Scancode.kp_4; Sdl.Scancode.kp_5;
    Sdl.Scancode.kp_6; Sdl.Scancode.kp_1;
    Sdl.Scancode.kp_2; Sdl.Scancode.kp_3;
    Sdl.Scancode.right; Sdl.Scancode.kp_period;
    Sdl.Scancode.a; Sdl.Scancode.z;
    Sdl.Scancode.e; Sdl.Scancode.r
]
...
let display_byte x y b =
    ...
    for j = 0 to 7 do
        ...
        acc := !acc || (oldv && v) ;
        let rect = Sdl.Rect.create (xp * scale) (yp * scale) scale scale in
...
let op_table = [
    ...
    fun op ->
        let x = shift_and_mask op 2 0xF in
        let bol = match shift_and_mask op 0 0xFF with
            | 0x9E -> true
            | 0xA1 -> false
            | _ -> invalid_instruction op
        in
            ...
            | 0x0A -> (* LD Vx, K *)
                ...
                while !value = 0xFF do
                    ...
                    if event_type = Sdl.Event.key_down then
                        ...
                        List.iteri (fun i x -> if x = scancode then value := i) keyboard_map
                done ;
                set_register x 0x0
            | 0x15 -> (* LD DT, Vx *)
                delay_timer := rx
...
let load_rom path =
    if path = "" then
        let default = [
            ...
            0x8C; 0x00; 0x7C; 0x05; 0xDC; 0x15; 0x70; 0x01;
            0x12; 0x08
        ] in
        ...
        Bytes.iteri (fun i el -> Array.set memory (0x200 + i) (int_of_char el)) store ;
...
let main =
    ...
    let window = match Sdl.create_window ~w:width ~h:height "Puce8" Sdl.Window.opengl with
        ...
        | Ok w -> w
    in
    let r = match Sdl.create_renderer window with
        | Error _ -> assert false
        | Ok r -> r
    in
    renderer := Some r ;
    ...
    while !continue do
        ...
        while Sdl.poll_event (Some event) do
            let event_type = Sdl.Event.get event Sdl.Event.typ in
            if event_type = Sdl.Event.key_down then
                match Sdl.Scancode.enum @@ Sdl.Event.get event Sdl.Event.keyboard_scancode with
                | `Return -> (* RESET *)
                    program_counter := 0x200 ; clear_screen ();
                    for x = 0 to 63 do
                        Array.fill screen.(x) 0 31 false
                    done
